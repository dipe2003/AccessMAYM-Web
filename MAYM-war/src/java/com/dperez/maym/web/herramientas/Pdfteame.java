 ...
 */
@Named
public class Pdfteame implements Serializable {

    // 1 pulgada = 72
    private Document documento = new Document(PageSize.A4, 72, 36, 36, 36);

    public Pdfteame(boolean horizontal) {
        if (horizontal) {
    ...
    public void ExportarListadoPD(String nombreArchivo, String tituloListado, List<Accion> acciones,
        ...
        try {
            ...
            TablaEncabezado event = new TablaEncabezado(CrearEncabezado(tituloListado));
            ...
            for (Accion a : acciones) {
                ...
                tabla.addCell(CrearCeldaContenido(a.getAnalisisCausa(), impar, 0));
                // Celda ACTIVIDADES
                if (a.getActividadesDeAccion().size() > 0) {
                    ...
                    tabla.addCell(CrearCeldaVacia(3, impar));
                    tabla.setComplete(true);
                }
    ...
    private PdfPCell CrearTituloImpresion(String titulo) {
        ...
        Chunk texto = new Chunk(titulo.toUpperCase());
        f.add(texto);
